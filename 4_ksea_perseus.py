...
usedirs = os.listdir(p_dex_dir)

for dex_dir in usedirs:
    fulldir = os.path.join(p_dex_dir, dex_dir)
    ...
    ttops = dict([f.lstrip('DEX_').rstrip('.txt'),
                  pd.read_csv(os.path.join(fulldir, f), sep='\t')]
                  for f in dex_files)
