...
raw_data = read_delim('data/20190919_PhosphoSTY_expandsites_4sets_withPtTitles_Basicfiltered_locProb_corrected.txt', '\t',
...
raw_df$ID = paste(paste(raw_data[, 'T: Protein'],
                        raw_data[, 'C: Amino acid'], sep='_'),
                  raw_data[, 'N: Position'], sep='')
...
targets = targets[targets$condition!="NA", ]

write.csv(targets, 'data/targets.csv', row.names=F)

aux = raw_df$ID
sample_cols = sample_cols[colnames(raw_df) %in% targets$sample]
...
batches = gsub('_.*', '', sample_cols)#gsub('Intensity ', '',  sample_cols))
...
for(b in unique(batches)){
    subdf = df[, colnames(df)[batches == b]]
