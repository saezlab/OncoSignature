'''
LASSO Signature - Approach D
============================

Here we use the training samples from sets 2 to 5 and train the models only
with the p-sites which are non-zero for all the validation samples from
the validation sets 1 and 2.
'''
...
for c in val:
  aux[c] = set(data.loc[data[c] != 0, c].index)

usesites = reduce(set.intersection, aux.values())
