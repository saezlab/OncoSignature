...
raw_df = as.data.frame(raw_data[, 40:47])
raw_df$ID = paste(paste(raw_data$Protein, raw_data$`Amino acid`, sep='_'),
                  raw_data$Position, sep='')

# Simplifying column names
colnames(raw_df) = gsub(' QBnorm*', '', colnames(raw_df))
colnames(raw_df) = gsub('Intensity Non_responder 006_020', 'A_NR', colnames(raw_df))
colnames(raw_df) = gsub('Intensity Responder 0015_018', 'B_R', colnames(raw_df))
colnames(raw_df) = gsub('Intensity Responder 0057_005', 'C_R', colnames(raw_df))
colnames(raw_df) = gsub('Intensity Non_responder 0207_007', 'D_NR', colnames(raw_df))
