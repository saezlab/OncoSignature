...
r2r1 = pd.read_csv('results/2_diff_exp/R2_R1_ttop.csv',
                   index_col=0, usecols=[0, 1, 4])
nr2nr1 = pd.read_csv('results/2_diff_exp/NR2_NR1_ttop.csv',
                     index_col=0, usecols=[0, 1, 4])
r1nr1 = pd.read_csv('results/2_diff_exp/R1_NR1_ttop.csv',
                    index_col=0, usecols=[0, 1, 4])
...
contrasts = {'R2_R1': r2r1, 'NR2_NR1': nr2nr1, 'R1_NR1': r1nr1}

for k, v in contrasts.items():
    print k
    #aux = v[abs(v['logFC']) > 1].copy()
    #aux['sort'] = abs(aux.logFC)
    #aux.sort_values(by='sort', ascending=False, inplace=True)
    #display(aux.sort_values(by='P.Value', ascending=True).head())
    aux = v[v['P.Value'] < 0.05].copy()
    aux['sort'] = abs(aux.logFC)
    
    display(aux.sort_values(by='sort', ascending=False).head())
...
def mapper(psites):
    ...
    psites_ens = []
    for i in psites:
        if i.startswith('CON__'):
            i = i[5:]
            
        if i.startswith('ENSEMBL:'):
            i = i[8:]
            ensembl.append(i.split('_')[0])
            psites_ens.append(i)

        else:
            upids.append(i.split('_')[0])
            psites_up.append(i)
            

    site_up = pd.DataFrame({'PSite':psites_up,
                            'ACC':upids})
    site_ens = pd.DataFrame({'PSite':psites_ens,
                             'ENSEMBL_PRO_ID':ensembl})

    up_keggid = up_map(site_up['ACC'], target='KEGG_ID')
    site_up_keggid = pd.merge(site_up, up_keggid, on='ACC', how='outer')
    
    if not site_ens.empty:
        ens_keggid = up_map(site_ens['ENSEMBL_PRO_ID'], target='KEGG_ID', source='ENSEMBL_PRO_ID')
        site_ens_keggid = pd.merge(site_ens, ens_keggid, on='ENSEMBL_PRO_ID', how='outer')
    
        site_id_keggid = pd.DataFrame({'PSite':site_up_keggid['PSite'].tolist()
                                               + site_ens_keggid['PSite'].tolist(),
                                       'ID':site_up_keggid['ACC'].tolist()
                                            + site_ens_keggid['ENSEMBL_PRO_ID'].tolist(),
                                      'KEGG_ID':site_up_keggid['KEGG_ID'].tolist()
                                                + site_ens_keggid['KEGG_ID'].tolist()})
    else:
        site_id_keggid = pd.DataFrame({'PSite':site_up_keggid['PSite'].tolist(),
                                       'ID':site_up_keggid['ACC'].tolist(),
                                       'KEGG_ID':site_up_keggid['KEGG_ID'].tolist()})
    ...
    return pd.merge(site_id_keggid,
                    keggid_keggpath,
                    on='KEGG_ID',
                    how='outer')
