...
for i, (train, test) in enumerate(sampler.split(x, y)):
    ...
    model = Lasso(Cs=np.linspace(1e-2, 1e0, 1000), sampler='skf', n_jobs=8)
...
m = model.coef_[0]
xx = x.iloc[sample, :].values

print model.predict_proba([xx])
print model.predict_log_proba([xx])
...
rng = np.linspace(0, 1, 101)


# In[11]:


plt.plot(rng, np.log(rng))
...
#x.index
print model.predict_proba(x.iloc[test, :])[:, 1]
print model.predict_proba(x.iloc[test, :])
#x.iloc[test, :]
...
for p in predictors.columns:
    ps.extend(predictors.loc[pd.notna(predictors[p]), p].index)
