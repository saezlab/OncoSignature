...
xmin = min(df_raw['slx dose (nM)'])
xmax = max(df_raw['slx dose (nM)'])
...
fig.legend([Patch(color='C%d' %i) for i in range(df.shape[1])],
           df.columns)
fig.tight_layout()
fig.savefig('results/A_dose_response/norm_survival.pdf')
...
fig.tight_layout()
fig.savefig('results/A_dose_response/fit_survival.pdf')
...
ax.legend([Patch(color=i) for i in ['r', 'y', 'g', 'b']],
          [r'$EC_{50}>10^{25}$', r'$10^{25}>EC_{50}>10^{9}$',
           r'$10^{9}>EC_{50}>10^{4}$', r'$EC_{50}<10^{4}$'], loc=0)

fig.tight_layout()
fig.savefig('results/A_dose_response/ec50.pdf')


# **NOTE:** $y$-axis is shown in $\log$ scale and has been trimmed at $10^{25}$ to avoid hindering visualization (some values are over $10^{92}$ or even $+\infty$).
# 
...
# - In yellow, values between $10^{25}nM$ and $10^{9}nM$. These are responding slightly to the treatment but *could* also be regarded as non-responders.
# - In green, values between $10^{9}nM$ and $10^{4}nM$. These samples are definitely responding to the treatment and are classified as responders.
# - In blue, values below $10^{4}nM$. These are samples which are also responders AND we have experimental proof of their $EC_{50}$.
